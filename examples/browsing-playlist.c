...
load_plugins ()
{
  ...
  /* Load plugin */
  if (!grl_registry_load_plugin_by_id (registry, "grl-playlist", &error))
    g_error ("Failed to load plugin: %s", error->message);

  source = grl_registry_lookup_source (registry, "grl-playlist");
  if (!source)
...
config_plugins (gchar* base_path)
{
  ...
  /* Configure plugin */
  config = grl_config_new ("grl-playlist", "Playlist");
  grl_config_set_string (config, "base-playlist", base_path);
      ...
      gchar  *argv[])
{
  ...
  if (argc != 2) {
    g_printf ("Usage: %s <base playlist>\n", argv[0]);
    return 1;
  }
